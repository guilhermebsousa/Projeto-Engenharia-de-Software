...
from api.schemas import LoginIn, UserOut, ProductIn, ProductOut, MovementIn, ChatbotIn, ChatbotOut
...
# ---- CHATBOT AI ----
@app.post("/api/users/{user_id}/chatbot", response_model=ChatbotOut)
def chatbot_query(user_id: str, payload: ChatbotIn, db: Session = Depends(get_db)):
    """Endpoint para processar perguntas do chatbot AI"""
    try:
        # Importar o agente AI
        import sys
        import os
        sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        from agente.ai_stock_agent import AIStockAgent
        ...
        return ChatbotOut(response=response)
        
    except Exception as e:
        # Em caso de erro, retornar uma resposta de fallback
        return ChatbotOut(response=f"Desculpe, ocorreu um erro ao processar sua pergunta: {str(e)}")
